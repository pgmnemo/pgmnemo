...
| `pgmnemo.track_recall_recency` | `on` | ✅ **PROVEN** — `mark_stale()` requires `last_recalled_at`; disabling prevents corpus curation. Zero performance impact on idle rows. | v0.9.5 design |
...
| `pgmnemo.confidence_boost_weight` | `0.0` | ✅ **PROVEN** — pre-registered gate: r_pb=+0.200 p=0.008, chi² NS p=0.47, power 29%. Gate requires STRONG evidence grade; current evidence is MODERATE. Stays 0.0 opt-in until gate clears. See ROADMAP §v1.0. | `benchmarks/METRICS_BY_VERSION.md` (v0.13.0 row) |
