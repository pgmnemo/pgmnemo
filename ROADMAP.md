...
| **v0.10.1–v0.16.1** | *(see CHANGELOG for individual entries)* | Content classifier, curation tools, entity-key extraction, failure-vocab widening, GUC evidence audit | 2026-06-20 – 2026-07-31 (✅ SHIPPED) |
| **v0.17.0** | Evidence integrity + foreign-schema CI gates + GUC consistency reconciliation | Anti-fabrication gate (`check_evidence_integrity.py`), foreign-schema gate (proves extension works outside our dev corpus), `graph_proximity_weight` COALESCE default unified to `0.0` across all code paths | 2026-08-13 (✅ SHIPPED) |
