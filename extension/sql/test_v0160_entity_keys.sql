...
-- pg_regress tests for pgmnemo v0.16.0 — extract_entity_keys() + ingest hook
--
-- Coverage:
--   A1: extract_entity_keys exists — 1 param, text[], immutable, strict
--   B1: project keys — "project:agency"
--   B2: model keys — "model:claude-sonnet-4-6"
--   B3: file keys — "file:apps/v3-next/routes/tasks.py"
--   B4: failure keys — "failure:PHANTOM_DONE"
--   B5: schema keys — "schema:pgmnemo.agent_lesson"
--   B6: multiple keys in one text
--   B7: no keys → empty array
--   B8: NULL input → NULL (STRICT)
...
-- ─────────────────────────────────────────────────────────────────────────────
-- B: extract_entity_keys — correct extraction per category
-- ─────────────────────────────────────────────────────────────────────────────
...
-- B6: multiple keys in one text
SELECT pgmnemo.extract_entity_keys(
    'When using claude-sonnet-4-6 on project:pgmnemo, the file apps/v3-next/workflows/w1_dispatch.py had a STALE_SESSION_ERROR that corrupted pgmnemo.agent_lesson records.'
) AS b6_multiple;
...
-- B8: NULL input → NULL (STRICT)
SELECT pgmnemo.extract_entity_keys(NULL) AS b8_null;
...
BEGIN
    _id := pgmnemo.ingest(
        ...
        'When deploying project:agency, check apps/v3-next/routes/tasks.py for broken imports.'::TEXT,
...
BEGIN
    _id := pgmnemo.ingest(
        ...
        'The model claude-sonnet-4-6 is preferred for project:agency dispatch tasks.'::TEXT,
