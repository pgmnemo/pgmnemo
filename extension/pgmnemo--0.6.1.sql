-- pgmnemo 0.6.1 — flat install script (fresh CREATE EXTENSION target)
-- Squashes the full upgrade chain 0.0.1 → 0.6.1 into a single idempotent DDL file.
-- Incorporates 0.4.1 base schema + 0.5.0 additions (R5/R6/R10/H-06/H-07/bitemporality).
-- Upgrade path (ALTER EXTENSION pgmnemo UPDATE TO '0.5.0') remains supported via:
--   pgmnemo--0.4.1--0.5.0.sql (incremental migration).
-- Generated 2026-05-23 — do not edit manually; maintain via upgrade scripts.
-- v0.6.1: F2 as_of_ts bitemporal filter + F3 stress test fixtures. F1 RRF deferred to v0.6.2.
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
    ...
    scored AS (
        SELECT
            ...
            -- RRF diagnostic (returned for analysis; NOT the primary ranking key)
            (vec_weight  / (_rrf_k_f + r.vec_rank::DOUBLE PRECISION)
           + bm25_weight / (_rrf_k_f + r.bm25_rank::DOUBLE PRECISION))
                AS rrf_diag,
            -- linear fusion: primary ranking signal (unchanged from v0.5.1/v0.6.0)
