-- pgmnemo--0.21.0.sql
-- Flat install for pgmnemo 0.21.0 (pg_extension_config_dump: user-data tables registered for pg_dump)
-- Generated from: pgmnemo--0.20.0.sql + pgmnemo--0.20.0--0.21.0.sql
...
COMMENT ON COLUMN pgmnemo.agent_lesson.last_recalled_at IS
    'Timestamp of the most recent recall across all recall paths '
    '(recall_hybrid, recall_lessons, navigate_locate, navigate_expand). '
    'NULL = never recalled since v0.9.5 column addition. '
    'Stamped automatically when pgmnemo.track_recall_recency = on (default). '
    'Primary input to pgmnemo.mark_stale() usage-based curation.';
COMMENT ON COLUMN pgmnemo.agent_lesson.recall_count IS
    'Cumulative recall count since v0.9.5 column addition. '
    'Monotonically increasing; never decremented. '
    'Stamped by the same recall paths as last_recalled_at.';
...
-- v0.9.5: recency index for stale-lesson queries
CREATE INDEX IF NOT EXISTS ix_pgmnemo_agent_lesson_last_recalled
    ON pgmnemo.agent_lesson (last_recalled_at)
    WHERE is_active AND last_recalled_at IS NOT NULL;

-- v0.9.6: sparse index for DAG-origin queries and exclude_dag_id filter
...
-- ─────────────────────────────────────────────────────────────────────────────
-- v0.9.6: memory_ingest_log — migration batch tracking
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- v0.21.0: pg_extension_config_dump — register user-data tables and their
-- sequences so pg_dump includes their contents in backups.
--
-- Tables included:   agent_lesson, mem_edge, memory_ingest_log
-- Tables excluded:   agent_lesson_state_transition (static extension data,
--                    seeded by the extension SQL itself — registering it would
--                    cause duplicate-key errors on restore)
-- Sequences included: one per BIGSERIAL table (preserves id continuity after
--                    restore; without this, sequences reset to 1 on CREATE EXTENSION)
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
    ...
    graph_walk (anchor_id, depth, reached_id, visited) AS (
        SELECT id, 0, id, ARRAY[id]  -- D1(v0.19.0): cycle guard
        FROM anchors
...
COMMENT ON FUNCTION pgmnemo.recall_lessons(vector, INT, TEXT, INT, TEXT) IS
    ...
    'δ = pgmnemo.graph_proximity_weight (default 0.0 since v0.10.1 — graph walk is opt-in per #88 ablation; v0.17.0 reconciled three overloads that still carried 0.2: navigate_locate(5p), recall_hybrid(10p), recall_hybrid(11p)). Range 0.0–0.5. Set to 0.2 to restore graph weighting.';
...
BEGIN
    ...
    graph_walk(anchor_id, depth, reached_id, visited) AS (
        SELECT cand_id, 0, cand_id, ARRAY[cand_id] FROM anchors  -- D1(v0.19.0): cycle guard
        UNION ALL
        SELECT gw.anchor_id, gw.depth + 1, me.target_id, gw.visited || me.target_id
        FROM graph_walk gw
        JOIN pgmnemo.mem_edge me ON me.source_id = gw.reached_id
...
-- =============================================================================
-- B) pgmnemo.reinforce() -- GUC-configurable confidence update (v0.9.3/D1)
...
DECLARE
    ...
    _raw_blend_weight   DOUBLE PRECISION;  -- v0.8.1 F2: cardinal raw score blend
    _track_recency      BOOLEAN;           -- v0.9.5: recall-recency stamping
BEGIN
    -- v0.9.5: read track_recall_recency GUC (default TRUE)
    BEGIN
    ...
    -- v0.9.5: stamp recalled lessons via data-modifying CTE
    ...
    _stamp AS (
        ...
        WHERE _track_recency
          AND id = ANY(ARRAY(SELECT r.id FROM result_set r))
        RETURNING id
    )
...
COMMENT ON FUNCTION pgmnemo.navigate_locate(vector, TEXT, INT, JSONB, INT) IS
    'Token-economy navigation LOCATE (v0.9.5). '
    'v0.9.5: VOLATILE (was STABLE PARALLEL SAFE); stamps last_recalled_at + recall_count '
    'on returned lessons via data-modifying CTE, gated by pgmnemo.track_recall_recency GUC (default TRUE). '
...
BEGIN
    -- v0.9.5: read track_recall_recency GUC
    BEGIN
        _track_recency := COALESCE(
...
COMMENT ON FUNCTION pgmnemo.navigate_expand(BIGINT[], TEXT[], INT, FLOAT, TEXT[]) IS
    'Token-economy navigation EXPAND (v0.9.5). '
    'v0.9.5: VOLATILE (was STABLE PARALLEL SAFE); stamps last_recalled_at + recall_count '
    'on returned lessons via data-modifying CTE, gated by pgmnemo.track_recall_recency GUC (default TRUE). '
...
-- =============================================================================
-- I1 OVERRIDE: recall_hybrid — restores confidence_boost_weight GUC support.
-- The v0.7.1 (migration chunk above) and v0.8.2-F2 redefinitions overwrote the
-- I1 body without _conf_boost_w. This final CREATE OR REPLACE is the canonical
-- installed version for CREATE EXTENSION pgmnemo VERSION '0.9.3'.
-- =============================================================================
...
-- =============================================================================
-- v0.9.6 FINAL OVERRIDES: recall_hybrid, recall_lessons, mark_stale
-- These must appear LAST in the flat install to supersede older versions
-- defined earlier in this file. All three are VOLATILE (recency stamping).
-- =============================================================================

-- =============================================================================
-- B.1) recall_hybrid() v0.9.6 — adds exclude_dag_id TEXT DEFAULT NULL (R13)
-- Must DROP older 8-arg overload because we're adding a parameter.
-- =============================================================================
...
-- =============================================================================
-- B.2) recall_lessons() v0.9.6 — adds exclude_dag_id TEXT DEFAULT NULL (R13)
...
BEGIN
    ...
    BEGIN
        _bm25_budget_ms := GREATEST(50, COALESCE(
    ...
    -- graph_walk is inherently bounded (5 anchors × depth 2 = ≤10 direct hops; D3 v0.19.0)
    graph_walk(anchor_id, depth, reached_id, visited) AS (
        SELECT id, 0, id, ARRAY[id] FROM anchors WHERE _graph_weight > 0  -- Fix 5; D1(v0.19.0): cycle guard
        UNION ALL
        SELECT gw.anchor_id, gw.depth + 1, me.target_id, gw.visited || me.target_id
...
-- ─────────────────────────────────────────────────────────────────────────────
-- v0.11.0 additions (typed recall — RFC-001 §D2 / P0.2)
-- ─────────────────────────────────────────────────────────────────────────────
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────
-- pgmnemo 0.12.0 additions (Typed Write API: remember_fact/event/relation)
-- ─────────────────────────────────────────────────────────────────────────
...
-- ═══════════════════════════════════════════════════════════════════════════
-- pgmnemo 0.12.2 → 0.13.0 delta applied inline
-- ═══════════════════════════════════════════════════════════════════════════
...
-- NOTE: p_used has NO DEFAULT — use 2-param shim below for backward compat (p_used=NULL).
-- No default avoids ambiguity with reinforce(BIGINT, TEXT) during overload resolution.
CREATE OR REPLACE FUNCTION pgmnemo.reinforce(
    p_lesson_id  BIGINT,
    p_outcome    TEXT,
    p_used       BOOLEAN
...
-- NOTE: p_used has NO DEFAULT — use 2-param shim below for backward compat.
CREATE OR REPLACE FUNCTION pgmnemo.reinforce(
    p_lesson_ids  BIGINT[],
    p_outcome     TEXT,
    p_used        BOOLEAN
...
COMMENT ON FUNCTION pgmnemo.consolidate(REAL, BOOLEAN, TEXT, INT) IS
    ...
    'Added in v0.14.0.';
-- End of pgmnemo--0.13.0--0.14.0.sql

-- =============================================================================
-- P1-A  consolidate() — v0.14.1: prior_state recorded in edge metadata
-- =============================================================================
...
-- pgmnemo 0.16.0-D: extract_entity_keys + recall_entity
-- Pruned categories: project:<slug> removed (noise), file: filtered (no reports/md)
-- Added: recall_entity() — entity-keyed recall via metadata->'entity_keys'
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 0.17.0 → 0.18.0: Auto-promotion (draft → validated by confirmed benefit)
-- 0.18.0 → 0.19.0: Graph walk repairs (D1 cycle guard, D2 UNION ALL, D3 depth cap 5→2; R-U1 recall_entity stamp removal)
-- ─────────────────────────────────────────────────────────────────────────────
