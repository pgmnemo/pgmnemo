-- pgmnemo--0.11.0.sql
-- Flat install: pgmnemo 0.11.0
-- Generated from pgmnemo--0.10.1.sql + pgmnemo--0.10.1--0.11.0.sql
-- SPDX-License-Identifier: Apache-2.0
--
-- Install: CREATE EXTENSION pgmnemo;
-- Upgrade: ALTER EXTENSION pgmnemo UPDATE TO '0.11.0';
--
-- v0.11.0 (ADR-61 §3 D3 / P0.2): typed recall
--   Single change: add optional param p_content_types text[] DEFAULT NULL (LAST)
--   to pgmnemo.recall_hybrid. NULL → unchanged behavior (all content types;
--   full backward compat). non-NULL → pushes content_type = ANY(p_content_types)
--   into BOTH subplans (vec + BM25) BEFORE RRF fusion — uses
--   ix_pgmnemo_content_type_active (pushdown, not post-filter).
--   '{}' → zero rows (explicit empty array, no silent fallback to all-types).
--   Inherits all v0.10.1 (#87) fixes: query_text cap, indexed BM25,
