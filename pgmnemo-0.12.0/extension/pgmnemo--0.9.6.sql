-- pgmnemo--0.9.6.sql
-- Flat install: pgmnemo v0.9.6
-- SPDX-License-Identifier: Apache-2.0
--
-- Squashes the full upgrade chain 0.0.1 → 0.9.6 into a single idempotent DDL file.
-- v0.9.6: R11/R12/R13 — versioned skill items, DAG-scoped recall, migration log
--         item_kind/version_n/patch_count/source_dag_id on agent_lesson;
--         pgmnemo.memory_ingest_log table;
--         exclude_dag_id param on recall_hybrid() and recall_lessons().
-- v0.9.5: E — Recall-recency signals + mark_stale()
--         RFC-PGM-CURATE-260619, 2026-06-19
--         E1: last_recalled_at + recall_count columns on agent_lesson
--         E2: Recall stamping in recall_hybrid, recall_lessons, navigate_locate, navigate_expand
--             GUC: pgmnemo.track_recall_recency (BOOLEAN, default TRUE)
--         E3: pgmnemo.mark_stale() — usage-based curation primitive
-- v0.9.4: documentation-only release.
-- v0.9.3: D1 reinforce() base-rate-adjusted deltas (+0.02/-0.12) + GUC-configurable.
-- v0.9.2: I1 flag-gated confidence-weighted recall ranking (pgmnemo.confidence_boost_weight).
...
-- Generated 2026-06-19 — do not edit manually; maintain via upgrade scripts.
-- ─────────────────────────────────────────────────────────────────────────────
