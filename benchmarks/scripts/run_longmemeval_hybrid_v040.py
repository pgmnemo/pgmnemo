...
Differences from run_longmemeval_pgmnemo.py (v0.2.1_pgmnemo_20260509 baseline):
  ...
  4. Output versioned as v0.3.0_<date>
...
def main():
    ...
    # CRITICAL: bge-m3 default max_seq_length=8192 (per HF config). For MPS this is
    # ~16x slower than necessary; LongMemEval sessions average ~2300 chars (~575 tokens),
    # so 512 tokens captures 99%+ of content with massive speedup.
    ...
    metrics = {
        "version": "v0.4.0_hybrid", "date": run_date,
        "dry_run": False, "mode": "real",
        "retrieval_method": "pgmnemo.recall_hybrid() vec_weight=0.4 bm25_weight=0.4 RRF k=60",
    ...
    out_dir = out_base / f"v0.4.0_hybrid_{run_date_compact}"
