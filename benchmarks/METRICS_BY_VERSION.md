...
| v0.13.0 (analytical) | 2026-07-12 | 0.13.0 | 0.4762 | 0.8814 | **0.9334** | 0.9853 | 0.8472 | **neutral — analytical carry-forward** through v0.11.0–v0.12.2; v0.13.0 adds posterior confidence + p_min_score (NULL default) + use_count — default install is byte-identical: `confidence_boost_weight=0.0` unchanged, `p_min_score=NULL` means no filtering, recall_lessons() ORDER BY key unmodified; pre-registered gate PASS (r_pb=+0.20, p=0.008) on production outcome data | carry-forward from `longmemeval/results/v0.3.0_20260513/` |
| v0.20.0 (analytical) | 2026-08-20 | 0.20.0 | 0.4762 | 0.8814 | **0.9334** | 0.9853 | 0.8472 | **neutral — analytical carry-forward** through v0.14.x–v0.19.x; v0.20.0 adds graph entity expansion (Variant C) + BFS causal expansion (Variant A) + `retrieval_source` output column — all new GUCs default to 0.0; guards short-circuit all new CTEs to empty sets at defaults; scoring formula reduces to v0.19.1 algebraically; Δrecall@K=0.0 by proof (BENCH_GATE PASS, gate/v0.20.0.json). Quality ablation of variants at enabled weights deferred to PREREG_020_EXPAND (SHA 4eb3a2b8). | carry-forward from `longmemeval/results/v0.3.0_20260513/` |
